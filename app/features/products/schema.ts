import { bigint, integer, jsonb, pgTable, text, timestamp, uuid, primaryKey, check } from "drizzle-orm/pg-core";
...
export const reviews = pgTable("reviews", {
  ...
  created_at: timestamp().notNull().defaultNow(),
  updated_at: timestamp().notNull().defaultNow(),
}, (table) => [check('rating-check', sql`${table.rating} BETWEEN 1 AND 5`)])
