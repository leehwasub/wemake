    ...
    <div className="space-y-10">
      <div className="flex items-center gap-4">
        <Avatar className="size-40">
          ...
          <AvatarFallback className="text-2xl">{loaderData.user.name.charAt(0)}</AvatarFallback>
        </Avatar>
      ...
      <div className="flex gap-5">
        {
          [
            ...
            <NavLink end key={item.label} className={({isActive}) => cn([buttonVariants({variant: "outline"}), isActive && "bg-red-100 text-foreground font-bold"])} to={item.to}>{item.label}</NavLink>
          ))
        }
      </div>
